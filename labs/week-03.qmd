...
Since $\lambda | \mathbf{T} \sim \Gamma\left(1 + n, 10 + \displaystyle\sum_{i=1}^{n} T_i\right)$ by conjugacy, we can also compute the conditional mean and standard deviations for $\lambda$ given observed samples. Namely,

$$
\mathbb{E}(\lambda | \mathbf{T}) = \frac{1 + n}{10 + \displaystyle\sum_{i=1}^{n} T_i}
$$

and

$$
\text{SD}(\lambda | \mathbf{T}) = \frac{\sqrt{1 + n}}{10 + \displaystyle\sum_{i=1}^{n} T_i}.
$$

The conditional mean for $\lambda$ evaluates to `{python} f"{11/122.3:.4f}"`, and the standard deviation evaluates to `{python} f"{np.sqrt(11)/122.3:.4f}"`.
...
This aligns closely enough with our analytical results above. We can also simulate predicted checkout times to verify our conclusions pertaining to the posterior predictive.
...
predictive = Predictive(checkout_model, posterior_samples=posterior_samples)
rng = jax.random.PRNGKey(1)

pred = predictive(rng, checkout_times=None)
t_pred = pred["obs"]

q = jnp.quantile(t_pred, jnp.array([0.025, 0.05, 0.5, 0.95, 0.975]))
print("Posterior predictive checkout time (Predictive):")
print(f"mean={t_pred.mean():.3f}, sd={t_pred.std():.3f}")
print(f"2.5%={q[0]:.3f}, 5%={q[1]:.3f}, median={q[2]:.3f}, 95%={q[3]:.3f}, 97.5%={q[4]:.3f}")
```
...
posterior_pdf = np.exp(gamma_dist.logpdf(jnp.array(x), 11.0, scale=1/122.3))

hist_counts, bin_edges = np.histogram(
    np.array(posterior_samples["lambda"]),
    bins=50,
    density=True,
)
...
posterior_sample_mean = float(jnp.mean(posterior_samples["lambda"]))
posterior_analytical_mean = 11.0 / 122.3
...
fig.add_trace(go.Scatter(
    x=x, y=np.array(posterior_pdf),
    mode="lines", name="Posterior (analytical): Γ(11, 122.3)",
    line=dict(color="#67A902", width=2.5),
))
fig.add_vline(
    x=posterior_sample_mean, line_dash="dot", line_color="#FD730A",
    annotation_text=f"Sample mean = {posterior_sample_mean:.4f}",
    annotation_font_color="#FD730A",
)
fig.add_vline(
    x=posterior_analytical_mean, line_dash="solid", line_color="#67A902",
    annotation_text=f"Analytical mean = {posterior_analytical_mean:.4f}",
...
posterior_sample_mean = float(jnp.mean(posterior_samples["lambda"]))
posterior_analytical_mean = 11.0 / 122.3
sorted_samples = jnp.sort(posterior_samples["lambda"])
...
sns.histplot(
    np.array(posterior_samples["lambda"]), bins=50, stat="density",
    color="#FD730A", alpha=0.55, edgecolor="white", linewidth=0.5,
    label="Posterior samples", ax=ax,
)

ax.axvline(posterior_sample_mean, linestyle=":", color="#FD730A", linewidth=2.5,
           label=f"Sample mean = {posterior_sample_mean:.4f}")
ax.axvline(posterior_analytical_mean, linestyle="-", color="#67A902", linewidth=2.5,
           label=f"Analytical mean = {posterior_analytical_mean:.4f}")
