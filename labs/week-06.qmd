...
**Write an integer program** that minimizes the total attention cost across all widget–region assignments. State the objective function, all constraints (each widget must be placed in exactly one region, and each region must hold exactly one widget), and the variable domains.
