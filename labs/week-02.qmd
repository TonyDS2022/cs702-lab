...
::: {.callout-tip title="Answer" icon=false}
### Expected utility for each design

We compute the expected utility for each design as
$$
\begin{aligned}
\mathbb{E}\bigg[U\big(D_{\text{Maximax}}\big)\bigg] &= 0.3 \times 9 + 0.4 \times 3 + 0.3 \times 6 = 5.7, \\
\mathbb{E}\bigg[U\big(D_{\text{Maximin}}\big)\bigg] &= 0.3 \times 5 + 0.4 \times 7 + 0.3 \times 4 = 5.5
\end{aligned}
$$

Since the optimal decision in the Minimax regret criterion resolves to one of $D_1$ or $D_2$, the expected utility would be either $5.7$ or $5.5$.

Since the Maximax criterion yields the highest expected utility, the optimal decision is $D_{\text{Maximax}}$.
:::
...
::: {.callout-tip title="Answer" icon=false}
Since asking the user incurs a utility cost of 1, the expected utility of asking the user is
$$
\mathbb{E}\bigg[U\big(D_{\text{Ask}}\big)\bigg] = 0.3 \times (9 - 1) + 0.4 \times (7 - 1) + 0.3 \times (8 - 1) = 6.9 > 5.7.
$$

Since the expected utility of asking the user is higher than the expected utility of immediately selecting the design with the highest expected utility, the agent should ask the user.
:::
...
```{python}
#| eval: true
def maximin_decision(utilities):
    min_utilities = [min(row) for row in utilities]
    best_design = max(range(len(min_utilities)), key=lambda i: min_utilities[i])
    return best_design, min_utilities[best_design]


def expected_utility(utilities_row, probabilities):
    return sum(u * p for u, p in zip(utilities_row, probabilities))


def expected_utility_decision(utilities, probabilities):
    eu_values = [expected_utility(row, probabilities) for row in utilities]
    best_design = max(range(len(eu_values)), key=lambda i: eu_values[i])
    return best_design, eu_values[best_design]


# Probability distribution from Q2
probabilities = [0.3, 0.4, 0.3]

# Maximin decision
design, value = maximin_decision(utilities)
print(f"Maximin: D{design + 1} with min utility = {value}")

# Expected utility for each design
print("\nExpected utilities:")
for i, row in enumerate(utilities):
    eu = expected_utility(row, probabilities)
    print(f"  E[U(D{i + 1})] = {eu:.1f}")

# Expected utility decision
design, value = expected_utility_decision(utilities, probabilities)
print(f"\nExpected Utility Decision: D{design + 1} with E[U] = {value:.1f}")
```
:::